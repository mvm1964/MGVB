...
int toSQL_proteins(sqlite3 *db, char *data_tbl) {
    ...
    //system(str);
    if (sqlite3_prepare_v2(db, "create index prot_id on proteins(prot_id, gene, header);", -1, &ppStmt, NULL)) {
...
int toSQL_mod_pep(sqlite3 *db, char *data_tbl, char *cnfg_file) {
    ...
    // 18.12.2023: will refactor to make a signle table for searching by scorer
    // now we use left joins at each spectrum which is slowing down. Will combine
    // peptides and mod_pep into a single table and index it by mass including all
    // necessary columns in the index
    
    sprintf(str, "create table if not exists mod_pep_new as select * from mod_pep left join peptides \
		    on mod_pep.pep_id=peptides.rowid order by mod_pep.mass;");
    
    if (sqlite3_prepare_v2(db, str, -1, &ppStmt, NULL)) {
        printf("Error preparing sql statement for mod_pep_new loading to sqlite db\n");
        sqlite3_close(db);
        exit(-1);
    }
    //sqlite3_prepare_v2(db, str, -1, &ppStmt, 0);
    ...
    // debug
    printf("Created mod_pep_new\n");

    // now create index
    // 28.12.2023: not needed anymore. Will just export the sorted table to be used by the new scorer
    /*sprintf(str, "Create index mod_mass on mod_pep_new(mass, pep_id, sequence, prot_id, \
		    missed, decoy, contam");
    for (int j = 0; j < i; j++) {
        strcat(str, ", ");
	strcat(str, ptm[j].name);
    }
    strcat(str, ");");
    sqlite3_prepare_v2(db, str, -1, &ppStmt, 0);
    sqlite3_step(ppStmt);
    ...
    // Export new mod pep table
    sprintf(str,  "select * from mod_pep_new order by mass;");
    FILE *f = fopen("mod_pep.db", "w");
    char *errs = NULL;
    if (!f) {
        printf("Cannot open mod_pep.db file for writing!\n");
	exit(1);
    }
    if (sqlite3_exec(db, str, callback, f, &errs) != SQLITE_OK) {
    ...
    sprintf(str, "drop table mod_pep;");
    if (sqlite3_prepare_v2(db, str, -1, &ppStmt, NULL)) {
        printf("Error preparing sql statement to drop mod_pep table!\n");
        sqlite3_close(db);
        exit(-1);
    }
    sqlite3_step(ppStmt);
    ...
    sprintf(str, "vacuum;");
    if (sqlite3_prepare_v2(db, str, -1, &ppStmt, NULL)) {
        printf("Error preparing sql statement to vacuum!\n");
        sqlite3_close(db);
        exit(-1);
    }
    sqlite3_step(ppStmt);
